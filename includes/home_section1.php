...
<?php 
        ...
        if (isset($_GET['category'])) {
          ...
          for ($i=0; $i <4 ; $i++) {    
            ...
            $query = "SELECT * FROM posts WHERE post_status = 'published' AND specific_section = '1' ORDER BY post_id DESC LIMIT 1";
                ...
                while ($row = mysqli_fetch_assoc($select_all_posts_query)) {
                    ...
                    $post_content = mb_substr($row['post_content'],0,260, "utf-8");
                    ...
                    <div class="big_post col-lg-6 col-md-6 col-sm-6 col-xs-6"><!-- big_post -->
						<div class="block_img_post"><!-- block_img_post -->
							<img src="images/news_images/<?php echo $post_image; ?>" alt="Image">
						</div><!-- // block_img_post -->
...
<?php 
        ...
        if (isset($_GET['category'])) {
          ...
          for ($i=0; $i <4 ; $i++) {    
            ...
            $query = "SELECT * FROM posts WHERE post_status = 'published' AND specific_section = '1' ORDER BY post_id DESC LIMIT 5 offset 1";
                $select_all_posts_query = mysqli_query($connection, $query);
}
                while ($row = mysqli_fetch_assoc($select_all_posts_query)) {
                    $post_id = $row['post_id'];
